...
def findPhraseInFiles(encoding="utf-8"):
    from pathlib import Path
    folder_path = input("Nhập đường dẫn thư mục: ").strip()
    phrase = input("Nhập cụm từ cần tìm: ").strip()
            ...
            with file_path.open("r", encoding=encoding, errors="ignore") as f:
                for line_num, line in enumerate(f, start=1):
                    if phrase in line:
                        ...
                        print(f"Tìm thấy trong file: {full_path}")
    ...
    if not found_any:
        print("Không tìm thấy cụm từ trong bất kỳ file nào.")

# Tiện ích xóa CRLF trong file văn bản
def clear_CRLF_in_file():
    from pathlib import Path

    file_path = input("Nhập đường dẫn file cần xóa CRLF: ").strip()
    input_file = Path(file_path)
    if not input_file.exists():
        print(f"File không tồn tại: {input_file}")
        return
    output_file = Path(file_path + "_clear_crlf.txt")
    ...
    print(f"Đã xóa CRLF và lưu file mới tại: {output_file.resolve()}")

def screen_capture_tool(file: str = "", verbose: bool = False) -> None:
    import subprocess
    from pathlib import Path

    if verbose:
        print("\nLàm theo hướng dẫn\n"
              "Nếu có thắc mắc liên hệ qbquangbinh@gmail.com\n")
    subprocess.run(file, shell=True)

    return
    
def cliHelp(help_file: str = "") -> None:
    try:
        with open(help_file, "r", encoding="utf-8") as f:
            print(f.read(), end="")
    except FileNotFoundError:
        print(f"Khong tim thay file: {help_file}")
    except OSError as e:
        print(f"Khong mo duoc file {help_file}: {e}")